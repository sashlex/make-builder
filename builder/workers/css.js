...
module.exports = async data => {
   ...
   await Promise.all( data.paths.map( async v => {
      ...
      path = dist.type === 'directory' ? Path.normalize( `${ v.dist}/${ srcBasename.substr( 0, srcBasename.lastIndexOf( '.' )) + '.css' }` ) : v.dist; // add filename into path with replace file extension

      return readFile( v.src, 'utf8' )
         .then( str => Less.render( str, { compress: true }))
         .then( result => writeFile( path, result.css ));

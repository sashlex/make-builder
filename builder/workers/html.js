...
module.exports = async data => {
   ...
   await Promise.all( data.paths.map( async v => {
      ...
      path = dist.type === 'directory' ? Path.normalize( `${ v.dist}/${ srcBasename.substr( 0, srcBasename.lastIndexOf( '.' )) + '.html' }` ) : v.dist; // add filename into path with replace file extension
      return writeFile( path, Nunjucks.render( v.src )); // promise
